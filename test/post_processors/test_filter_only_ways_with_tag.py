...
def test_all_roads_are_roads(raymond_road, highway_roads):
    data = [raymond_road]
    expected_result = [copy(raymond_road)]

    result = list(FilterOnlyWaysWithTag(tag='highway', values=highway_roads).process(data))
...
def test_no_surface_tag_is_paved(raymond_road, surfaces_paved):
    data = [raymond_road]
    expected_result = [copy(raymond_road)]
...
def test_no_surface_tag_is_unpaved(raymond_road, surfaces_paved):
    data = [raymond_road]
    expected_result = [ [ copy(raymond_road[0]) ],
                        [ copy(raymond_road[2]),
                          copy(raymond_road[3]) ] ]
...
def test_alternating_paved_unpaved_with_unpaved_surfaces(barnes_road, surfaces_paved):
    data = [barnes_road]
